...
class Cliente:
    ...
    def realizar_transacao(self, conta, transacao):
        # Valida o numero de transações e invalidar a operação se for necessário
        if len(conta.historico.trasacoes_do_dia()) >= 2:
            print('Você execedeu o número de transações permitida para hoje!')
            return
...
class Historico:
    ...
    def transacoes_do_dia(self): # Filtrar todas as transações realizadas do dia
        data_atual = datetime.now().date()
        transacoes = []
        # Itera sobre as transações existentes
        for transacao in self.transacoes:
            # Obtém a data da transação formatada como string no formato desejado
            data_transacao = datetime.strftime('%d-%m-%Y %H:%M:%S').date()

            # Compara a data atual com a data da transação    
            if data_atual == data_transacao:
                # Se as datas coincidirem, adiciona a transação à lista de transações do dia
                transacoes.append(transacao)
            # Retorna a lista de transações do dia
            return transacoes
